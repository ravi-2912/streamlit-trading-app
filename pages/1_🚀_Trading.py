...
with st.spinner("Loading data..."):
    ...
    if "symbols_df" not in st.session_state:
        st.session_state["symbols_df"] = get_all_items_from_table(
            Symbol, ["id", "symbol", "description"]
        )
    if "strategies_df" not in st.session_state:
        st.session_state["strategies_df"] = get_all_items_from_table(
            Strategy, ["id", "name"]
        )
...
with st_col2:
    ...
    strategy = c1.selectbox(
        "Strategy",
        options=st.session_state["strategies_df"]["Name"],
        accept_new_options=True,
        placeholder="Select or write new strategy",
    )
    trade_risk = 0.01 * c2.number_input(
        "Trade Risk %",
        min_value=0.1,
        max_value=3.0,
        value=1.0,
    ...
    with st.expander("Common Parameters", expanded=True):
        ...
        common_sl = c1.number_input(
            "Stop Loss (pips)", step=1.0, min_value=5.0, value=15.0
        )
        common_tp = c2.number_input(
            "Take Profit (pips)", step=1.0, min_value=5.0, value=15.0
        )
        common_dir = c3.selectbox(
            "Direction", options=[direction.value for direction in DirectionType]
        )
        common_order_type = c4.selectbox(
            "Order Type", options=[order_type.value for order_type in OrderType]
        )
        dir_currency = c5.selectbox(
            "Direction based on Currency",
            options=["", *[currency.value for currency in CurrencyType]],
        )

    symbols = st.multiselect(
        "Select Symbols", options=st.session_state["symbols_df"]["Symbol"]
    )
    ...
    if symbols:
        ...
        def highlight_factor_cell(row):
            ...
            if round(row["SL Pips"], 2) != round(expected_sl, 2) or round(
                row["TP Pips"], 2
            ) != round(expected_tp, 2):
                return [
                    "background-color: red" if col == "SL/TP Factor" else ""
        ...
        st.session_state["edited_df"] = st.data_editor(
            ...
            column_config={
                ...
                "SL/TP Factor": st.column_config.NumberColumn(
                    "SL/TP Factor", default=1, help="Multiplier for Common SL/TP"
                ),
                "Direction": st.column_config.SelectboxColumn(
                    "Direction", options=["Long", "Short"], required=True
                ),
                "Order Type": st.column_config.SelectboxColumn(
                    "Order Type", options=["Market", "Limit", "Stop"], disabled=True
                ),
                "Entry Price": st.column_config.NumberColumn(
                    "Entry Price",
                    disabled=True if common_order_type == "Market" else False,
                ),
                "SL Pips": st.column_config.NumberColumn(
                    "SL Pips", default=common_sl, help="Override Common SL"
                ),
                "TP Pips": st.column_config.NumberColumn(
                    "TP Pips", default=common_tp, help="Override Common TP"
                ),
